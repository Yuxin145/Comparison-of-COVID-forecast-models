...
rt_prior_dist <- LogNormal(meanlog = 0.079, sdlog = 0.18)

# estimate R
estimates <- epinow(data %>% dplyr::select(date, cases) %>% rename(confirm = cases),
                    generation_time = gt_opts(dist = generation_time),
                    backcalc = backcalc_opts(prior_window = 7),
                    delays = delay_opts(incubation_period + reporting_delay),
                    rt = rt_opts(prior = rt_prior_dist, use_breakpoints = F, pop = 19453561),
                    gp = gp_opts(basis_prop = .2, alpha = Normal(mean = 0, sd = .1)),
                    stan = stan_opts(cores = 4, samples = 2000, warmup = 500, 
                                     control= list(adapt_delta= 0.95)),
                    forecast = forecast_opts(horizon=21),
                    output = "fit")

estimates_rw <- epinow(data %>% dplyr::select(date, cases) %>% rename(confirm = cases),
                    generation_time = gt_opts(dist = generation_time),
                    backcalc = backcalc_opts(prior_window = 7),
                    delays = delay_opts(incubation_period + reporting_delay),
                    rt = rt_opts(prior = rt_prior_dist, use_breakpoints = F, pop = 19453561),
...
estimates_bc <- epinow(data %>% dplyr::select(date, cases) %>% rename(confirm = cases),
                    generation_time = gt_opts(dist = generation_time),
                    backcalc = backcalc_opts(prior_window = 7),
                    delays = delay_opts(incubation_period, reporting_delay),
                    rt = NULL,
...
ex_pred <- train_and_predict("epinow2", data = data, seed = 12345, d = 10, n = 21,
                             pop = cc$pop[cc$state_id=="ny"]) %>%data.frame() 

ex_pred <- ex_pred %>% 
  mutate(k = 1:21) %>% 
  reshape2::melt("k") 

ex_pred <- ex_pred %>% mutate(variable = as.numeric(gsub("X", "", variable)))
