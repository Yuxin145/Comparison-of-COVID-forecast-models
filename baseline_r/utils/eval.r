...
source("D:/Users/peter/Desktop/UROPS/covid predict code/settings/naming.R")
...
compute_incidence <- function(df, model, cap_fcast = c(0, 1e5), min_inc = -Inf, sum_by = "week") {
  ...
  df_by <- left_join(observedDF, forecastDF, by = c("state", "date")) %>%
    ...
    mutate(model = model,
           model = recode(model, !!! model_names_all),
           model = factor(model, model_names_all),
...
pred_score <- function(
  ...
  if (type == "calibration") {
    ...
    scoring_fct <- function(x, y = NULL, q = NULL) {
      xx <- as.numeric(unlist(x))
      scoringutils::mad_sample(predicted = matrix(xx, nrow = 1)) 
    }
    ...
    scoring_fct <- function(x, y, alphas) {
      y     <- as.numeric(y[1])
      qvals <- as.numeric(x)
      qlevs <- as.numeric(alphas)
      stopifnot(length(qvals) == length(qlevs))
      
      as.numeric(scoringutils::wis(
        observed       = y,
        predicted      = qvals,
        quantile_level = qlevs,
        na.rm          = TRUE        # <-- drop unpaired levels like 0.55
      ))
    }
  }
