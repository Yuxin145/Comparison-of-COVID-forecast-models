  ...
  for (mod in models) {
    for (j in 1:length(states)) {
      ...
      for (k in 1:K) {
        ...
        #test_df_state$data[[k]]$forecast <- get_samples(predicted, test_df_state$data[[k]])
        test_k <- nrow(test_df_state$data[[k]])
        test_df_state$data[[k]]$forecast <- get_samples(
          predicted, test_df_state$data[[k]], k = min(test_k, n_preds)
        )
      }
