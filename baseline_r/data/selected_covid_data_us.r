...
# plot the data of the selected states (together on one 'hchart' graph)
...
# plot the data of the selected stats in the selected time period
...
phase_labs <- read_csv("data/us-selected-states_labeled-phases.csv") %>%
  ...
  mutate(epidemic_phase = str_to_sentence(epidemic_phase)) %>%
