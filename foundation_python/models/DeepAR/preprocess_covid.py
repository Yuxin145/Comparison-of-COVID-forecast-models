...
def prep_data(data, covariates, data_start, train = True):
    ...
    input_size = window_size-slide_size
    windows_per_series = np.full((num_series), (time_len-input_size) // slide_size)
    
    if train: # decideds whether this method training or testing dataset
        windows_per_series -= (data_start+slide_size-1) // slide_size
    ...
    for series in trange(num_series):
        series_len = time_len - data_start[series]
        
        # changed
        if train:
            if series_len > 0:
                cov_age = stats.zscore(np.arange(series_len))
                covariates[data_start[series]:time_len, 0] = cov_age[:series_len]
        else:
            if series_len > 0:
                cov_age = stats.zscore(np.arange(series_len))
                covariates[:, 0] = 0.0
                covariates[data_start[series]:, 0] = cov_age

        for i in range(windows_per_series[series]):
            if train:
                window_start = slide_size*i+data_start[series]
            else:
                window_start = slide_size*i
...
if __name__ == '__main__':
    ...
    data_path = "data/covid_incidence.csv"
    data_frame = pd.read_csv(data_path, index_col=0, parse_dates=True)

    covariates = gen_covariates(data_frame.index, num_covariates)
