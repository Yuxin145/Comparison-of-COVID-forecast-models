    ...
    with torch.no_grad():
      ...
      for i, (test_batch, id_batch, v, labels) in enumerate(tqdm(test_loader)):
          ...
          if sample:
              ...
              all_samples_1.append(samples.cpu().numpy())      # (S, batch, horizon) 新增
      ...
      SAMPLES = np.concatenate(all_samples_1, axis=1)  # (S, N_windows, horizon)
      SAMPLES = np.transpose(SAMPLES, (1, 2, 0)) # (N_windows, horizon, S)
      np.save(os.path.join(params.model_dir, "forecast_samples_sequential.npy"), SAMPLES)
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join('experiments', args.model_namepython evaluate.py --dataset covid --sampling) 
    ...
    test_loader = DataLoader(test_set, batch_size=params.predict_batch, sampler=SequentialSampler(test_set), num_workers=4)
    logger.info('- done.')
